...
bank_account = decimal.Decimal(0)       # начальная сумма 0
...
def ext():
    ...
    if bank_account > RICHNESS_SUM:
        rich_per = bank_account * RICHNESS_PERCENT
        bank_account -= rich_per
        operations.append(
            f'Вычтен налог на богатство {RICHNESS_PERCENT}% в сумме {rich_per} у.е. Итого {bank_account} у.е.'
        )
    operations.append(
        f'Возьмите карту на которой {bank_account} у.е.'
    )
