...
logging.basicConfig(filename='log/file_objects.log',
                    filemode='a',
                    encoding='utf-8',
                    level=logging.INFO,
...
def get_file_objects(directory):
    ...
    for entry in os.scandir(directory):
        ...
        name = os.path.splitext(entry.name)[
            0] if entry.is_file() else entry.name
        # расширение (если это файл)
        extension = os.path.splitext(entry.name)[
            1] if entry.is_file() else None
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Принимаем пусть до директории")
...
# python t_1.py -d C:\Users\Lenovo\MyPython\Python-Immersion\work_15\task_15
