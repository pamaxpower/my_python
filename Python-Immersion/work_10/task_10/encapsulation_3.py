'''
Приватные модификаторы (private)

Переменная __max_up доступна внутри класса и его экземпляров.
Мы используем её для увеличения количества жизней персонажа в методе 
add_up - проблем с доступом нет.
Но если обратиться к свойству напрямую, получаем ошибку доступа к
атрибуту.
'''
