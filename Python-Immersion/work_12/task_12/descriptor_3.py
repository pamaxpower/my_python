'''
Класс-дескриптор Range
'''

class Range:
    """
    класс-дескриптор, реализующий базовые методы чтения, записи и удаления
    """
    def __init__(self, min_value: int = None, max_value: int = None):
        """
        инициализируем класс с параметрами для минимального и максимального значения
        :param min_value: по умолчанию None
        :param max_value: по умолчанию None
        """
        self.min_value = min_value
        self.max_value = max_value

    def __set_name__(self, owner, name):
        """
        Контроль имени: определение имени для свойства
        :param owner: пользователь
        :param name: имя переменной
        :return:
        """
        # создается защищенная переменная
        self.param_name = '_' + name

    def __get__(self, instance, owner):
        """
        Контроль получения значений
        :param instance: экземпляр класса
        :param owner:
        :return:
        """
        # читает атрибут self.param_name у экземпляра instance
        return getattr(instance, self.param_name)

    def __set__(self, instance, value):
        """
        Контроль присвоения значений
        :param instance:
        :param value:
        :return:
        """
        # вызывает внутренний метод validate
        self.validate(value)
        # присваивает значение value атрибуту self.param_name экземпляру instance
        setattr(instance, self.param_name, value)

    def __delete__(self, instance):
        """
        Котроль удаления атрибутов
        :param instance:
        :return:
        """
        # при попытке удаления вызываем ошибку
        raise AttributeError(f'Свойство "{self.param_name}" нельзя удалять')
    
    def validate(self, value):
        """
        Метод валидации значений
        :param value: значение
        :return:
        """
        # проверка на целое число
        if not isinstance(value, int):
            raise TypeError(f'Значение {value} должно быть целым числом')
        # проверка на то, что значение больше минимального
        if self.min_value is not None and value < self.min_value:
            raise ValueError(f'Значение {value} должно быть больше или равно {self.min_value}')
        # проверка на то, что значение меньше максимального
        if self.max_value is not None and value >= self.max_value:
            raise ValueError(f'Значение {value} должно быть меньше {self.max_value}')
    
class Student:
    age = Range(3, 103)
    grade = Range(1, 11 + 1)
    office = Range(3, 42 + 1)

    def __init__(self, name, age, grade, office):
        self.name = name
        self.age = age
        self.grade = grade
        self.office = office

    def __repr__(self):
        return f'Student(name={self.name}, age={self.age}, grade={self.grade}, office={self.office})'

if __name__ == '__main__':
    std_one = Student('Архимед', 12, 4, 29)
    # std_other = Student('Аристотель', 2406, 5, 17)    # ValueError: Значение 2406 должно быть меньше 103
    print(f'{std_one = }')
    std_one.age = 15
    print(f'{std_one = }')
    # std_one.grade = 11.0      # TypeError: Значение 11.0 должно быть целым числом
    # std_one.office = 73       # ValueError: Значение 73 должно быть меньше 42
    # del std_one.age           # AttributeError: Свойство "_age" нельзя удалять
    print(f'{std_one.__dict__ = }')
